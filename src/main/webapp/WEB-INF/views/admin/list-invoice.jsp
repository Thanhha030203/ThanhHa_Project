...
<div class="row">
    <div class="col-6 controller-invoice mt-5">
        ...
        <table class="table table-striped table-hover table-bordered table-invoice ms-4">
            ...
            <tbody>
                <c:forEach var="item" items="${pageInvoice.content}">
                    <tr>
                        <td>${item.invoiceCode}</td>
                        <td>${item.byerName}</td>
                        <td>${item.purchaseDate}</td>
                        <td>${item.totalInvoice}</td>
                        <td><a href="/invoice/edit/${item.invoiceCode}"><i class="bi bi-pencil-square"></i></a>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
    ...
    <div class="col-6 controller-detailInvoice mt-5">
        ...
        <table class="table table-striped table-hover table-bordered table-detailInvoice">
            ...
            <tbody>
                <c:forEach var="item" items="${itemDetailInvoice}">
                    <tr>
                        <td>${item.detailInvoiceCode}</td>
                        <td>${item.bookCode.bookTitle}</td>
                        <td>${item.number}</td>
                        <td>${item.total}</td>
                </c:forEach>
            </tbody>
        </table>
        <button class="btn-delete-invoice" formaction="/delete/invoice/"><i class="bi bi-trash"></i> Xóa</button>
    </div>
</div>
</form:form>